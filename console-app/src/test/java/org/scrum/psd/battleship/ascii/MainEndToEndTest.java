...
public class MainEndToEndTest {
    ...
    @Test
    public void testShootOutsideGridDoesntCrash(){
        try {
            gameInput.provideLines("a1", "a2", "a3", "a4", "a5", "b1", "b2", "b3", "b4", "c1", "c2", "c3", "d1", "d2", "d3", "e1", "e2", "z4", "a0", "a1");

            Main.main(new String[]{});
        } catch(NoSuchElementException e) {
            Assert.assertTrue(systemOutRule.getLog().contains("Welcome to Battleship"));
            Assert.assertTrue(systemOutRule.getLog().contains("Position cant be parsed."));
            Assert.assertTrue(systemOutRule.getLog().contains("Your shot is outside the grid, try again:"));
        }
    }
}
